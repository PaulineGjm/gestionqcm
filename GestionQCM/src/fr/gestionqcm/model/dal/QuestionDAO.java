...
public class QuestionDAO {
	...
	public static Question getQuestionById(Integer idQuestion) throws Exception {
		...
		try {
			...
			rqt = cnx.prepareStatement("select q.*, r.id_reponse"
			...
			while (rs.next()) {
				...
				if (question == null) {
					...
					question = new Question(idQuestion, idTheme, wording, isArchive, urlImage);
				}
			...
			throw new Exception("Probleme de connection avec la base de donn�es");
		} finally {
			if (rs != null)
				rs.close();
			if (rqt != null)
	...
	public static List<Question> getQuestionByTheme(Integer idTheme) throws Exception {
		PreparedStatement cmd = null;
		List<Question> questions = new ArrayList<Question>();
		cmd = AccessDatabase.getConnection().prepareStatement(requestFactory.getSelectOne(Columns.idTheme.getText()));
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("Problème de connexion avec la base de données !");
		} finally {
